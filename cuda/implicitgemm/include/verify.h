...
void direct_conv2dcpu_nhwc(float* input, float* filter, float* bias, float* output, int N, int C, int H, int W, int K, int R, int S, int U,int V,  int P, int Q)
{
    int Oh = (H + 2*P - R)/U + 1;
    int Ow = (W + 2*Q - S)/V + 1;
                    ...
                    for(int r = 0; r < R; r++)
                    {
                        for(int s = 0; s < S; s++)
                        {
                            ...
                            {                       
                                ...
                                {
                                    // sum += (input[n*C*H*W + c*(W*H)+ ih*W + iw] * filter[k*R*S*C + c*R*S + r*S + s]);
                                    sum += (input[n*C*H*W + ih*(W*C)+ iw*C + c] * filter[k*R*S*C + r*C*S + s*C + c]);
                                }
                            }                       
                        }
                    }
