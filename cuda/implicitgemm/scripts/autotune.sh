...
RUNNER="src/implgemm_8.cu"
OUTPUT="./benchmark_results/autotune_results.txt"
...
sed -i "s/const uint NUM_THREADS = .*/const uint NUM_THREADS = $NUM_THREADS;/" $RUNNER
...
sed -i "s/const uint wm = .*/const uint wm = $WM;/" $RUNNER
sed -i "s/const uint wn = .*/const uint wn = $WN;/" $RUNNER
sed -i "s/const uint wniter = .*/const uint wniter = $WN_ITER;/" $RUNNER
sed -i "s/const uint tm = .*/const uint tm = $TM;/" $RUNNER
sed -i "s/const uint tn = .*/const uint tn = $TN;/" $RUNNER
...
# timeout -v 8 ./implgemm8 1 256 384 512 256 3 3 1 1 1 1 0 0 0 | tee -a $OUTPUT
timeout -v 8 ./implgemm8 1 512 96 128 512 3 3 1 1 1 1 0 0 0 | tee -a $OUTPUT
done
done
done
done
